...
jags_parallel <- function(
    ...
    if(parallel$type == "PSOCK" || parallel$type == "FORK") {
        ...
        if(parallel$type == "PSOCK")
            cl <- parallel::makeCluster(n.clusters, methods = FALSE, 
                type = "PSOCK", outfile = outfile)

        if(parallel$type == "FORK")
            cl <- parallel::makeCluster(n.clusters, methods = FALSE, 
                type = "FORK", outfile = outfile)
...
jags_serial <- function(
    ...
    ## Burnin
    if(burnin > 0)
        update(model, n.iter = burnin, progress.bar = progress.bar)
